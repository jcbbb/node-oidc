...
import { ConflictError, InternalError, ResourceNotFoundError } from "../utils/errors.js";
...
export async function get_by_id(id) {
  ...
  if (!result.rows.length) {
    return [null, new ResourceNotFoundError({ key: "user_not_found", params: { identifier: id } })];
  }
...
export async function get_by_email(email) {
  ...
  if (!result.rows.length) {
    return [null, new ResourceNotFoundError({ key: "user_not_found", params: { identifier: email } })];
  }
